...
const { setIsExtended, isExtended } = inject('isExtended');
...
const handleClick = () => {
  if (props.mode === 'toggle') {
    ...
    setIsExtended(isExtended.value = !isExtended.value);
  }
  else toolBarButtonStore.setTab(props.label)

  if (toolBarButtonStore.getPressed(props.label)) drawerStore.openDrawer(props.tooltip)
  else drawerStore.closeDrawer()
}
</script>

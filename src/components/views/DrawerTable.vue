 ...
 */
const INITIAL_ITEMS = [
        {requestId:'VM-20241105-001',serviceType:'VM Deployment',requestSummary:'Provision Production Web Server (EU)',status:'Complete',completion:75,requestedOn:'2024-11-05T09:30:00Z',targetName:'prod-web-eu-01',currentStage:'Verification & Handoff',requestedBy:'alice.johnson@corp.com',timeToCompletion:'0 minutes (Completed)',outputResult:'VM IP: 10.15.2.14, Access Key issued.',linkedResources:'https://console.cloud.corp/vm/prod-web-eu-01',errorMessage:null,fullLogOutput:'Successfully deployed VM using Terraform v1.1.2. Firewall rules applied.',workflowPath:'DeployVM-Prod-Europe',lastUpdated:'2024-11-05T09:42:15Z'},
        {requestId:'NW-20241105-002',serviceType:'Network Change',requestSummary:'Open Port 443 for Payment Gateway',status:'Failed',completion:75,requestedOn:'2024-11-05T10:15:00Z',targetName:'SecGroup-Payments',currentStage:'Applying Policy Changes',requestedBy:'bob.brown@corp.com',timeToCompletion:'N/A',outputResult:'Policy update failed.',linkedResources:'https://jira.corp/TICKET-589',errorMessage:'Validation Error: Policy requires approval from Network Admin before deployment.',fullLogOutput:'Pre-flight check failed. Policy violation detected. Exit code 403.',workflowPath:'NetworkPolicy-Update',lastUpdated:'2024-11-05T10:16:30Z'},
        {requestId:'ST-20241105-003',serviceType:'Storage Provisioning',requestSummary:'Expand Analytics DB Storage to 5TB',status:'In Progress',completion:75,requestedOn:'2024-11-05T11:00:00Z',targetName:'analytics-db-disk',currentStage:'Resizing Disk Volume',requestedBy:'charlie.davis@corp.com',timeToCompletion:'Estimated 5 minutes',outputResult:'Volume resizing initiated.',linkedResources:'https://storage.corp/vol/analytics-db-disk',errorMessage:null,fullLogOutput:'Resizing initiated successfully. Monitoring growth rate...',workflowPath:'Storage-Resize-Standard',lastUpdated:'2024-11-05T11:03:00Z'},
        {requestId:'VM-20241105-004',serviceType:'VM Deployment',requestSummary:'Test Environment for API V3',status:'Cancelled',completion:75,requestedOn:'2024-11-05T12:45:00Z',targetName:'test-api-v3',currentStage:'Initial Queuing',requestedBy:'diana.evans@corp.com',timeToCompletion:'N/A',outputResult:'Request manually cancelled by requester.',linkedResources:null,errorMessage:'Cancelled by user.',fullLogOutput:'Request queued, but cancelled before execution began.',workflowPath:'DeployVM-Test-Standard',lastUpdated:'2024-11-05T12:46:10Z'},
        {requestId:'TK-20241105-005',serviceType:'Open Ticket',requestSummary:'Report: Slow VPN Connection',status:'Pending Support',completion:75,requestedOn:'2024-11-05T14:00:00Z',targetName:'IT Support Team',currentStage:'Assignment to Tier 1',requestedBy:'frank.green@corp.com',timeToCompletion:'SLA: 1 Hour for initial response',outputResult:'Ticket created successfully.',linkedResources:'https://servicedesk.corp/TICKET-1903',errorMessage:null,fullLogOutput:'Ticket submitted to the Service Desk queue.',workflowPath:'ServiceDesk-Intake',lastUpdated:'2024-11-05T14:00:45Z'},
        {requestId:'ST-20241105-006',serviceType:'Storage Provisioning',requestSummary:'Create S3 Bucket for Log Archive',status:'Complete',completion:75,requestedOn:'2024-11-05T15:35:00Z',targetName:'log-archive-2024-s3',currentStage:'Finalizing Permissions',requestedBy:'grace.hall@corp.com',timeToCompletion:'0 minutes (Completed)',outputResult:'Bucket ARN: arn:aws:s3::log-archive-2024',linkedResources:'https://console.cloud.corp/s3/log-archive-2024-s3',errorMessage:null,fullLogOutput:'Bucket created. Encryption and lifecycle policies applied.',workflowPath:'S3-Bucket-Standard',lastUpdated:'2024-11-05T15:37:12Z'},
        {requestId:'VM-20241105-007',serviceType:'VM Deployment',requestSummary:'High-Mem Test Server for QA',status:'Pending Approval',completion:75,requestedOn:'2024-11-05T16:10:00Z',targetName:'qa-mem-test-05',currentStage:'Awaiting Financial Approval',requestedBy:'ivan.jones@corp.com',timeToCompletion:'Estimated 24 hours (Approval SLA)',outputResult:'Pending Cost Center review.',linkedResources:'https://approval.corp/request/109',errorMessage:null,fullLogOutput:'Resource exceeds standard budget threshold. Routing to approver.',workflowPath:'DeployVM-Test-HighCost',lastUpdated:'2024-11-05T16:10:00Z'},
        {requestId:'NW-20241105-008',serviceType:'Network Change',requestSummary:'Assign Floating IP to Staging Load Balancer',status:'In Progress',completion:75,requestedOn:'2024-11-06T08:00:00Z',targetName:'stg-load-balancer-01',currentStage:'DNS Record Update',requestedBy:'kevin.lee@corp.com',timeToCompletion:'Estimated 15 minutes',outputResult:'Floating IP assigned.',linkedResources:'https://console.cloud.corp/lb/stg-load-balancer-01',errorMessage:null,fullLogOutput:'IP allocated and attached. Propagation pending.',workflowPath:'Network-FloatingIP-Assign',lastUpdated:'2024-11-06T08:05:20Z'},
        {requestId:'TK-20241105-009',serviceType:'Open Ticket',requestSummary:'New Access Request: HR System',status:'Complete',completion:75,requestedOn:'2024-11-06T09:30:00Z',targetName:'User: laura.martin@corp.com',currentStage:'Final Handoff',requestedBy:'laura.martin@corp.com',timeToCompletion:'0 minutes (Completed)',outputResult:'Access granted. Confirmation email sent.',linkedResources:'https://servicedesk.corp/TICKET-1904',errorMessage:null,fullLogOutput:'Access request approved by manager and fulfilled by automation.',workflowPath:'Access-Request-Standard',lastUpdated:'2024-11-06T09:31:50Z'},
        {requestId:'ST-20241105-010',serviceType:'Storage Provisioning',requestSummary:'Migrate Old File Share to Archive Tier',status:'Failed',completion:75,requestedOn:'2024-11-06T10:50:00Z',targetName:'fileshare-old-project',currentStage:'Data Transfer',requestedBy:'mike.nelson@corp.com',timeToCompletion:'N/A',outputResult:'Transfer failed after 20% completion.',linkedResources:null,errorMessage:'Storage Quota Exceeded: Archive tier capacity is full. Cannot transfer 5TB.',fullLogOutput:'Data transfer aborted at 20% due to destination capacity constraint. Check available space.',workflowPath:'Storage-Archive-Migrate',lastUpdated:'2024-11-06T11:20:00Z'}
];
</script>
 ...
 */
watch(activeFilterKey, (newVal, oldVal) => {
    if (newVal && !oldVal) { // Transition from closed (null) to open
    ...
    } else if (!newVal && oldVal) { // Transition from open to closed (null)
        document.removeEventListener('click', closeFilterOnOutsideClick, { capture: true });
    }
}, { immediate: true }); // Check initial state for robustness
                ...
                <div class="header-inner">
                  <div 
                    ...
                    <div v-if="header.sortable !== false" class="sort-icon-container">
                      <svg class="sort-arrow sort-asc" viewBox="7 9 10 5" fill="none" xmlns="http://www.w3.org/2000/svg" :class="{'is-active-dir': sortKey === header.value && sortDir === 'asc'}">
                        <path d="M7 14L12 9L17 14H7Z" fill="currentColor"/>
                      </svg>
                      <svg class="sort-arrow sort-desc" viewBox="7 10 10 5" fill="none" xmlns="http://www.w3.org/2000/svg" :class="{'is-active-dir': sortKey === header.value && sortDir === 'desc'}">
                  ...
                  <button 
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                      <path d="M14.072 13.918L21.414 6.574C22.195 5.793 21.652 4.5 20.579 4.5H3.42C2.348 4.5 1.805 5.793 2.586 6.574L9.928 13.918C10.026 14.016 10.081 14.148 10.081 14.288V20.5C10.081 21.328 10.871 21.758 11.536 21.353L13.149 20.375C13.737 20.02 14.081 19.387 14.081 18.694V14.288C14.081 14.148 14.026 14.016 14.072 13.918Z" stroke="none" stroke-width="2"/>
                    </svg>
                  </button>
                </div>
                  ...
                  <div 
                    ...
                    <div class="filter-controls">
                      ...
                      <button 
                        ...
                        type="button"
                        @click.stop="clearFilter(header.value); toggleFilter(header.value)"
              ...
              <td 
                ...
                <template v-else-if="header.value === 'completion'">
                  <div class="completion-bar-wrapper">
                    <div role="progressbar" :aria-valuenow="item.completion" aria-valuemin="0" aria-valuemax="100" class="completion-bar-container">
                      <div class="completion-bar" :style="{ width: item.completion + '%', backgroundColor: getCompletionColor(item.completion) }"></div>
                    </div>
                ...
                <template v-else-if="header.value === 'actions'">
                  <div class="actions-cell">
                    <button class="cancel-button" type="button" @click.stop="cancelItem(item)" :disabled="item.completion === 100" :class="{ 'disabled': item.completion === 100 }">
...
.header-content:focus-visible { /* Add outline for keyboard users */
...
.header-cell.justify-center .header-inner { justify-content: center; }
.header-cell.justify-right .header-inner { justify-content: flex-end; }
...
.filter-icon:hover { color: var(--text-default); }
.filter-icon svg { width: var(--filter-icon-size); height: var(--filter-icon-size); }
.filter-icon.is-active-filter { color: var(--accent-color); }
...
.clear-filter-button:hover { filter: brightness(1.1); }
...
.data-row:nth-child(even) { background-color: var(--table-bg-odd); }
.data-row:hover { background-color: var(--table-bg-hover); }
...
.cell-price { font-weight: 600; color: var(--status-success); }
.cell-rating { font-weight: 600; color: #ffc107; }
